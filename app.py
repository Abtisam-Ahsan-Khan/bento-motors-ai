...
available_makes = sorted(df["standard_make"].dropna().astype(str).str.strip().unique().tolist())
...
with col1:
    ...
    # pull ALL models for this make directly from the full dataset
    filtered_models = sorted(
        df.loc[df["standard_make"].astype(str).str.strip() == make, "standard_model"]
